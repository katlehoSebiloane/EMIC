...
public class EMERGENCY_TILE extends VBox {
    ...
    EMERGENCY_TILE(Stage PrimaryStage){
        ...
        grey_box.setStyle("-fx-background-color:#2d2d2d;-fx-background-radius:20px");
        grey_box.setSpacing(10);
        ID.setFont(Font.font("Segoe UI", FontWeight.EXTRA_LIGHT, FontPosture.ITALIC,16));
        ID.setFill(Color.WHITE);
        Hospital.setFill(Color.WHITE);
        Paramedic.setFill(Color.WHITE);
        Date.setFill(Color.WHITE);
        Date.setFont(Font.font("Segoe UI", FontWeight.EXTRA_LIGHT, FontPosture.ITALIC,16));
        Hospital.setFont(Font.font("Segoe UI", FontWeight.EXTRA_LIGHT, FontPosture.ITALIC,16));
        Paramedic.setFont(Font.font("Segoe UI", FontWeight.EXTRA_LIGHT, FontPosture.ITALIC,16));
        grey_box.getChildren().addAll(ID,Date,Hospital,Paramedic);
        setStyle("-fx-background-color:#DC2929;-fx-background-radius:20px");
        setPadding(new Insets(0,30,0,0));
        getChildren().add(grey_box);

        setOnMouseClicked(e->{
            try {
                PrimaryStage.setScene(new Scene( new EMIC_TRENDS(PrimaryStage),1920,1000));
